...
class DrQV2Agent:
    ...
    def set_dd(self, datasetDistillation):
        self.datasetDistillation = datasetDistillation
    ...
    def critic_grad(self, obs, action, reward, discount, next_obs, step):
        ...
        self.encoder_opt.zero_grad(set_to_none=True)
        self.critic_opt.zero_grad(set_to_none=True)
        grad = torch.autograd.grad(critic_loss, list(self.encoder.parameters()), create_graph=True)
        grad += torch.autograd.grad(critic_loss, list(self.critic.parameters()), create_graph=True)
        # critic_loss.backward()
    ...
    def update_on_sync(self, step):
        ...
        obs, action, reward, discount, next_obs = self.datasetDistillation.get_data()
